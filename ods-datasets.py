...
ods_datasets = requests.get(
    f"{base_url}{catalog_datasets_endpoint}", params=query_params
)
# %%
...
url_list = [
    f"{base_url}{catalog_datasets_endpoint}/{dataset_id}{export_endpoint}{export_format[i]}"
    for i, dataset_id in enumerate(datasets_id)
]
url_list
# %%
url_list_parquet = [
    f"{base_url}{catalog_datasets_endpoint}/{dataset_id}/exports/parquet"
...
def download_ods_parquet(
    ...
    if not valid_urls:
        ...
        for i, url in enumerate(url_list):
            ...
            r = requests.get(url, params=cred_param)
            with open(
                f"{save_folder}{datasets_id[i]}{geo_label[i]}.parquet", "wb"
            ) as f:
                f.write(r.content)
        return True
